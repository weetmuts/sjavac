...
public enum Option {
    ...
    VERSION("-version", "Print sjavac version") {
        @Override
        protected void processMatching(ArgumentIterator iter, OptionHelper helper) {
            System.out.println("sjavac version 0.4");
        }
    },
    ...
    MODULEPATH("-modulepath", "Nah! Just kidding!") {
        @Override
        protected void processMatching(ArgumentIterator iter, OptionHelper helper) {
            List<Path> paths = getFileListArg(iter, helper);
            if (paths != null)
    ...
    JJ("-jj", "Limit number of source compile chunks") {
        @Override
        protected void processMatching(ArgumentIterator iter, OptionHelper helper) {
            if (!iter.hasNext() || !iter.peek().matches("\\d+")) {
                helper.reportError(arg + " must be followed by an integer");
            ...
            helper.numCompileChunks(Integer.parseInt(iter.next()));
        }
    },
    ...
    STOPSERVER("-stopserver", "Stop the server and do not compile anything") {
        @Override
        protected void processMatching(ArgumentIterator iter, OptionHelper helper) {
            helper.stopServer();
        }
    ...
    PERMIT_ARTIFACT("-Xpermit-artifact:", "Allow this artifact in destination directory") {
        @Override
        protected void processMatching(ArgumentIterator iter, OptionHelper helper) {
    ...
    PERMIT_UNIDENTIFIED_ARTIFACTS("-Xpermit-unidentified-artifacts", "Allow unidentified artifacts in destination directory") {
        @Override
        protected void processMatching(ArgumentIterator iter, OptionHelper helper) {
            helper.permitUnidentifiedArtifacts();
        }
    },
    PERMIT_SOURCES_WITHOUT_PACKAGE("-Xpermit-sources-without-package", "Permit sources in the default package") {
        @Override
        protected void processMatching(ArgumentIterator iter, OptionHelper helper) {
            helper.permitDefaultPackage();
        }
    },
    COMPARE_FOUND_SOURCES("-Xcompare-found-sources", "Compare found sources with given sources") {
        @Override
        protected void processMatching(ArgumentIterator iter, OptionHelper helper) {
    ...
    STATE_DIR("-state-dir:", "Directory used to store the sjavac state file (defaults to dest-dir suffixed with _state)") {
        @Override
        protected void processMatching(ArgumentIterator iter, OptionHelper helper) {
    ...
    SERVER_DIR("-server-dir:", "Directory used to store portfile and log files (defaults to state-dir)") {
        @Override
        protected void processMatching(ArgumentIterator iter, OptionHelper helper) {
            String p = iter.current().substring(arg.length());
            helper.serverDir(Paths.get(p));
        }
    };
