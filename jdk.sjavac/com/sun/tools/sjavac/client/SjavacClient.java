...
public class SjavacClient implements Sjavac {
    ...
    public SjavacClient(Options options) {
        ...
        String base = options.getServerDir().toFile().getAbsolutePath()+File.separatorChar+"javac_";
        portfileName = (p!=null) ? p : base+"portfile";
        logfile = Util.extractStringOption("logfile", serverConf, base+"log");
        stdouterrfile = Util.extractStringOption("stdouterrfile", serverConf, base+"outerr");
        background = Util.extractBooleanOption("background", serverConf, true);
        String vmargs = Util.extractStringOption("vmargs", serverConf, "");
        if (!vmargs.equals("")) {
            vmargs = " "+vmargs;
        }
        String cmd = Util.extractStringOption("sjavac", serverConf);
        if (cmd == null) {
            // No explicit command was set, lets attempt to recreate the entire java launcher command line.
            cmd = ForkCommand.getCommandLine() + vmargs+" com.sun.tools.sjavac.Main";
            if (cmd == null) {
                // The java launcher command line could not be recreated!
                // Lets fall back on sjavac, and hope that it is to be found in the path.
                cmd = "sjavac";
            }
        }
        sjavacForkCmd = cmd;
    ...
    private Socket tryConnect() throws IOException {
        ...
        for (int i = 0; i < MAX_CONNECT_ATTEMPTS; i++) {
            Log.debug(String.format("Trying to connect (attempt %d of %d)",
            ...
            try {
                ...
                Log.timing("Connecting to server took "+(stop-start)+"ms");
                return socket;
    ...
    private boolean makeSureServerIsRunning(PortFile portFile)
        ...
        if (!portFile.containsPortInfo()) {
            String forkCmd = SjavacServer.fork(sjavacForkCmd,
                                               ...
                                               stdouterrfile,
                                               background);
            Log.debug("Spawning server: "+forkCmd);
    ...
    public void shutdown() {
        try (Socket socket = tryConnect()) {
            ...
            ObjectInputStream ois = new ObjectInputStream(socket.getInputStream());
            oos.writeObject(id);
            oos.writeObject(SjavacServer.CMD_SHUTDOWN);
            oos.flush();
    ...
    /**
     * Make a request to the server to get the class location (in a jar, a class file etc)
     */
    @Override
    public String getClassLoc(String c) {
        try (Socket socket = tryConnect()) {
            ...
            ObjectInputStream ois = new ObjectInputStream(socket.getInputStream());
            oos.writeObject(c);
            oos.writeObject(SjavacServer.CMD_GET_CLASS_LOC);
            oos.flush();
    ...
    /**
     * Make a request to the server to get the public api of a class.
     */
    @Override
    public PublicApiResult getPublicApi(String c) {
        try (Socket socket = tryConnect()) {
            ...
            ObjectInputStream ois = new ObjectInputStream(socket.getInputStream());
            oos.writeObject(c);
            oos.writeObject(SjavacServer.CMD_GET_PUBLIC_API);
            oos.flush();
            return (PublicApiResult) ois.readObject();
        } catch (IOException | ClassNotFoundException ex) {
            Log.error("[CLIENT] Exception caught: " + ex);
            ex.printStackTrace(System.err);
        }
